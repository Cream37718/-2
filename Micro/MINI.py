...
while True:
    ...
    if keyboard.is_pressed('r'):
        ...
        # หมุนมอเตอร์ซ้ายหลังจากถ่ายภาพ
        if ser and ser.is_open:
            ser.write(b'x')
            print("Motor started after capturing image")
            start_time = time.time()

            # ส่งข้อความทางไลน์ว่าผ้ากำลังถูกซักเมื่อมอเตอร์เริ่มหมุน
            send_line_message("Cream", "ผ้าของคุณกำลังถูกซัก")
            print("Message sent: ผ้าของคุณกำลังถูกซัก")

            # หมุนมอเตอร์เป็นเวลา 1 นาที
            while time.time() - start_time < 60:
                ser.write(b'l')  # สมมุติว่าคำสั่ง 'l' หมุนซ้าย
                time.sleep(1)

            print("Motor rotation completed")
        else:
            print("Motor not started: Serial port not available")
        break
